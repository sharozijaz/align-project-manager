...
export function PageHeader({ title, description, actions }: { title: string; description?: string; actions?: ReactNode }) {
  return (
    <div className="rounded-[var(--radius-lg)] border border-[var(--panel-border)] bg-[var(--panel-bg)] px-4 py-4 shadow-[var(--shadow-sm)] sm:px-5">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
        <div className="min-w-0 border-l-4 border-[var(--brand-primary)] pl-4">
          <h1 className="text-[1.7rem] font-black leading-tight tracking-normal text-[var(--text)]">{title}</h1>
