  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-4">
      {cards.map((card) => (
        <Card
          key={card.label}
          className="group overflow-hidden p-0 hover:-translate-y-px hover:border-[var(--border-strong)] hover:shadow-[var(--shadow-md)]"
          style={{
            background: `color-mix(in srgb, ${card.tone} 12%, var(--panel-bg))`,
            borderColor: `color-mix(in srgb, ${card.tone} 28%, var(--panel-border))`,
          }}
        >
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-start justify-between gap-3">
                ...
                <span
                  ...
                  style={{
                    background: `color-mix(in srgb, ${card.tone} 10%, var(--panel-bg))`,
                    borderColor: `color-mix(in srgb, ${card.tone} 36%, var(--panel-border))`,
                    color: card.tone,
                  }}
                >
                  {card.icon}
              ...
              <p className="mt-3 text-xs font-semibold text-[var(--text-muted)]">{card.helper}</p>
            </div>
          </div>
        </Card>
      ))}
