      ...
      <div className="mt-6 grid min-w-0 gap-3 lg:grid-cols-2">
        {active.length ? (
          active.map((project) => {
            ...
            return (
              <Link key={project.id} to={`/projects/${project.id}`} className="block min-w-0 overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--surface-raised)] p-4 transition hover:border-[var(--border-strong)] hover:bg-[var(--surface-hover)]">
                <div className="flex min-w-0 items-start justify-between gap-3">
                  <h3 className="min-w-0 break-words text-lg font-semibold text-slate-900">{project.name}</h3>
